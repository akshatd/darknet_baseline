...
// Layers includes so that declarations arent implicit
...
// For getting wall time while threading
#include <sys/time.h>
#include <unistd.h>
...
void test_yolo(char *cfgfile, char *weightfile, char *filename, float thresh)
{
	...
	while(1){
		char input_list[MAX_FILES][MAX_FILENAME];
		int num_files = 1;
		if(filename){
			parse_filenames(filename, input_list, &num_files);
		} else {
			printf("Enter Image Path: ");
			fflush(stdout);
			input = fgets(input, MAX_FILENAME, stdin);
			if(!input) return;
			strtok(input, "\n");
			strcpy(input_list[0], input);
		}
		...
		printf("Total Time taken is %f secondssssss\n", elapsed_sec(tstart, tend));
		if (filename) break;
	}
}
...
void parse_filenames(char *file_dir, char input_list[MAX_FILES][MAX_FILENAME], int *num_files){
	// if filename is a jpg in itself
	char *dot = strrchr(file_dir, '.');
	if ((dot && !strcmp(dot, ".jpg")) || (dot && !strcmp(dot, ".png"))){
		strcpy(input_list[0], file_dir);
		*num_files = 1;
	}
	// it is a directory, get all the files in it
	else{
		DIR           *d;
		struct dirent *dir;
		d = opendir(file_dir);
		if (d){
			int x = 0;
			char dir_path[MAX_FILENAME] = "\0";
			strcat(dir_path, file_dir);
			strcat(dir_path, "/");
			while ((dir = readdir(d)) != NULL && x<MAX_FILES){
				char *dot = strrchr(dir->d_name, '.');
				if ((dot && !strcmp(dot, ".jpg")) || (dot && !strcmp(dot, ".png"))){
					char full_name[MAX_FILENAME];
					strcpy(full_name, dir_path);
					strcat(full_name, dir->d_name);
					strcpy(input_list[x], full_name);
					x++;
				}
			}
			closedir(d);
...
float elapsed_sec(struct timeval start, struct timeval end){
...
float *forward_layer(layer l, network_state state){
...
#ifdef TIME_PROFILING_H
		end_s = clock();
		double seconds_s = sec(end_s - start_s);
		printf("%d-th Layer: scal_cpu time: %lf s\n",i, seconds_s);
#endif
	}
	if(l.type == CONVOLUTIONAL){
		...
		printf("%d-th Layer: forward_convolutional_layer: %lf s\n",i, seconds);
#endif
		...
		double seconds = sec(end - start);
		printf("%d-th Layer: forward_connected_layer: %lf s\n",i,seconds);
#endif
	} else if(l.type == RNN){
		...
		double seconds = sec(end - start);
		printf("%d-th Layer: forward_maxpool_layer: %lf s\n",i,seconds);
#endif
